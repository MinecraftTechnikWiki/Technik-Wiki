...
execute as @n[type=minecraft:marker,tag=TW_Eingabe] run data modify entity @s data.TW_Verlauf append from entity @s data.TW_Befehle[]

# Hat man mehr als 50 Einträge wird der erste Eintrag gelöscht.
execute as @n[type=minecraft:marker,tag=TW_Eingabe] store result score @s TW_Test run data get entity @s data.TW_Verlauf
data remove entity @n[type=minecraft:marker,tag=TW_Eingabe,scores={TW_Test=51..}] data.TW_Verlauf[0]
