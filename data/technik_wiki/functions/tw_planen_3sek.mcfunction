...
execute as @a unless score @s TW = @s TW run function technik_wiki:tw_nummer

execute as @a store result score @s TW_Test run clear @s minecraft:writable_book{TW_Befehlsbuch:true} 0
clear @a[scores={TW_Test=2..}] minecraft:writable_book{TW_Befehlsbuch:true}
execute as @a unless entity @s[scores={TW_Test=1}] run give @s minecraft:writable_book{TW_Befehlsbuch:true,display:{Name:'{"translate":"Befehlsbuch","color":"light_purple","italic":false}',Lore:['"Tippe hier einen Befehl ein"','"(ohne Enter) und drücke"','"Fertig. Beispiel: Hilfe um"','"eine Übersicht über alle"','"eigens für diese Welt"','"geschaffenen (Vanilla)"','"Befehle zu erhalten"'] } }
