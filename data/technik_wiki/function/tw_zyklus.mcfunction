...
execute at @a[tag=TW_Drop,scores={TW_Drop=1..}] run kill @n[distance=..1,type=minecraft:item,tag=TW_Symbol]
execute at @a[tag=TW_Drop,scores={TW_Drop=1..}] run data merge entity @n[distance=..3,type=minecraft:item] {Tags:["TW_Symbol","TW_Entfernen","TW_Zentrierung"],Age:-32768s,PickupDelay:32767s}
